...
MESSAGING_POLICY_FILE="$(mktemp)"
cp "$ROOT_DIR/infra/amplify-compute-messaging-policy.json" "$MESSAGING_POLICY_FILE"
aws iam put-role-policy \
  --role-name "$COMPUTE_ROLE_NAME" \
  --policy-name FamilyMessagingAccess \
  --policy-document "file://$MESSAGING_POLICY_FILE"
rm -f "$MESSAGING_POLICY_FILE"
echo "    Attached SES/SNS messaging policy"

COMPUTE_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/${COMPUTE_ROLE_NAME}"

SES_FROM_ADDRESS="${FAMILY_SES_FROM_ADDRESS:-noreply@${DOMAIN_NAME}}"
MESSAGING_DRY_RUN="${FAMILY_MESSAGING_DRY_RUN:-false}"
SMS_ENABLED="${FAMILY_SMS_ENABLED:-false}"

ENV_VARS="FAMILY_DDB_TABLE=${FAMILY_DDB_TABLE},FAMILY_USE_IN_MEMORY_DB=${FAMILY_USE_IN_MEMORY_DB:-false},FAMILY_LOGIN_ANSWER=${LOGIN_ANSWER},CANONICAL_HOST=${DOMAIN_NAME},NEXT_PUBLIC_SITE_URL=https://${DOMAIN_NAME},FAMILY_SESSION_SECRET=${SESSION_SECRET},FAMILY_SES_FROM_ADDRESS=${SES_FROM_ADDRESS},FAMILY_MESSAGING_DRY_RUN=${MESSAGING_DRY_RUN},FAMILY_SMS_ENABLED=${SMS_ENABLED}"
